def counting_sort(arr, k):
    ...
    counting_arr = [0] * (k+1)          #카운팅 배열

    #1. counting_arr에 arr 내 원소의 빈도수 체크
    for i in range(len(arr)):     # arr 만큼 순회 // arr의 원소 하나씩 꺼내기
    ...
    #4. result_arr에 정렬하기 (counting_arr를 참조할것이다.)
    for i in arr:       #arr는 순회 가능한 iterable 객체 -> collection
        counting_arr[i] -= 1        # 0, 4, 1, 3 ... : counting_arr의 해당 원소 값 하나 줄인다.
        result_arr[counting_arr[i]] = i     #result_arr에 counting_arr 의 해당 요소를 넣는다.
