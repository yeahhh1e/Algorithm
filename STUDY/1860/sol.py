...
for tc in range(1, T+1):
    ...
    # print(sorted_time)
    n = float(K/M) # 1초당 만들 수 있는 붕어빵의 갯수
    # print(a)
    ...
    cnt = 0   # 붕어빵 재고 수량
    sales = 0 # 판매 수량
    for i in range(1, arrival_time[-1]+1): # 마지막 손님이 도착하는 시간까지 만들기
        cnt += n    # 1초마다 붕어빵 만들기
        for j in arrival_time: # 손님이 도착한 시간과 현재 시간 비교
            if i == j:  # 손님이 도착했을 때
                if i < M:   # 다 못 만들었는데 손님이 도착했을 때
                    result = "impossible"
                    break
                elif cnt >= 1 and (j / M - sales) >= 1: # 1개 이상 남아있고 도착시간/만드는데 걸리는 시간 - 판매수량 >=1 이상일때만
                    cnt -= 1
                    sales += 1
                    arrival_time.pop(0)
