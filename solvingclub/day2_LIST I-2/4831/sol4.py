...
for tc in range(1, T+1):
    ...
    while current_idx + K < N:  # 종점에 도달할 때까지 반복
        next_stop = 0
        # 현재 위치에서 K만큼 이동할 수 있는 범위 내에서 가장 먼 충전소 찾기
        for i in range(1, K + 1):
            if bus_stop[current_idx + i]:
                next_stop = current_idx + i

        if next_stop == 0:  # 충전소를 찾지 못한 경우 (도달 불가)
            result = 0
            break
